...
var scanCmd = &cobra.Command{
  ...
  magellan scan --subnet 10.0.0.0/24 --subnet 172.16.0.0 --subnet-mask 255.255.0.0 --cache ./assets.db`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		switch format := viper.GetString("scan.format"); format {
		case "json", "yaml":
			var output []byte
			var err error
...
func init() {
	addFlag("insecure", scanCmd, "insecure", "", true, "Skip TLS certificate verification during probe")
	addFlag("scan.ports", scanCmd, "ports", "", []int{}, "Adds additional ports to scan for each host with unspecified ports.")
	addFlag("scan.scheme", scanCmd, "scheme", "", "https", "Set the default scheme to use if not specified in host URI. (default is 'https')")
	addFlag("scan.protocol", scanCmd, "protocol", "", "tcp", "Set the default protocol to use in scan. (default is 'tcp')")
	addFlag("scan.subnets", scanCmd, "subnets", "", []string{}, "Add additional hosts from specified subnets to scan.")
	addFlag("scan.subnet-mask", scanCmd, "subnet-mask", "", net.IPv4Mask(255, 255, 255, 0), "Set the default subnet mask to use for with all subnets not using CIDR notation.")
	addFlag("scan.disable-probing", scanCmd, "disable-probing", "", false, "Disable probing found assets for Redfish service(s) running on BMC nodes")
	addFlag("scan.disable-caching", scanCmd, "disable-caching", "", false, "Disable saving found assets to a cache database specified with 'cache' flag")
	addFlag("scan.format", scanCmd, "format", "F", "db", "Output format (db, json, yaml)")
	addFlag("scan.output", scanCmd, "output", "o", "", "Output file path (for json/yaml formats)")
	addFlag("scan.include", scanCmd, "include", "", []string{"bmcs"}, "Asset types to scan for (bmcs, pdus)")
